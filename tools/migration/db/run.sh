...
if [ "$(ls -A /var/lib/mysql)" ]; then
    ...
    if [ -e '/var/lib/mysql/PG_VERSION' ]; then
        ...
        ISMYSQL=true
        if [ "$(ls -A /var/lib/mysql/notaryserver)" ]; then
            ISNOTARY=true
        fi
    fi
fi
...
function up_notary {
    ...
    if [[ $(mysql $DBCNF -N -s -e "select count(*) from information_schema.tables \
        ...
        # delete all data to make it re-runable
        psql -U $PGSQL_USR -d notaryserver -t -c "TRUNCATE tuf_files;"
        psql -U $PGSQL_USR -d notarysigner -t -c "TRUNCATE private_keys;"
...
function alembic_up() {
    ...
    if [ $db_type = "mysql" ]; then
        ...
        alembic -c /harbor-migration/db/alembic.ini upgrade $target_version
        rc="$?"
        alembic -c /harbor-migration/db/alembic.ini current
