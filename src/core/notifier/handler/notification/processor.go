...
func constructImagePayload(event *notifyModel.ImageEvent) (*notifyModel.Payload, error) {
	...
	if repoRecord == nil {
		log.Debugf("cannot match repository info with repo %s", repoName)
	} else {
...
func preprocessAndSendImageHook(value interface{}) error {
	...
	if err != nil {
		log.Errorf("failed to match rule for %s event: %v", imgEvent.EventType, err)
		return err
	}
	// if cannot match rule including event type in project, return directly
	if len(policies) == 0 {
		log.Debugf("cannot match rule for %s event: %v", imgEvent.EventType, imgEvent)
		return nil
	}
