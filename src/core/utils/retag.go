...
func Retag(srcImage, destImage *models.Image) error {
	...
	srcClient, err := NewRepositoryClientForUIWithMiddler("harbor-ui", getRepoName(srcImage))
	if err != nil {
		return err
	}
	destClient := srcClient
	if !isSameRepo {
		destClient, err = NewRepositoryClientForUIWithMiddler("harbor-ui", getRepoName(destImage))
		if err != nil {
			return err
		}
	}
