...
func (r *referrersHandler) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	art, err := artifact.Ctl.GetByReference(ctx, repository, reference, nil)
	if err != nil {
	...
	total, err := accessory.Mgr.Count(ctx, query)
	if err != nil {
		lib_http.SendError(w, err)
		return
	}
	...
	for _, acc := range accs {
		accArt, err := artifact.Ctl.GetByReference(ctx, repository, acc.GetData().Digest, nil)
		if err != nil {
			lib_http.SendError(w, err)
			return
		}
