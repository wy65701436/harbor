...
func (m *managerTestSuite) TestGet() {
	...
	m.Require().Nil(err)
	m.Equal(int64(1), accessory.GetID())
	m.dao.AssertExpectations(m.T())
}

func (m *managerTestSuite) TestCreate() {
	mock.OnAnything(m.dao, "Create").Return(int64(1), nil)
	acc, _ := model.New(model.TypeCosignSignature, model.ArtifactID(1), model.Size(1))
	_, err := m.mgr.Create(nil, acc)
	m.Require().Nil(err)
	m.dao.AssertExpectations(m.T())
}
