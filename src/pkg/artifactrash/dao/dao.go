...
func (d *dao) Create(ctx context.Context, artifactrsh *model.ArtifactTrash) (id int64, err error) {
	...
	if err != nil {
		if e := orm.AsConflictError(err, "artifact trash %s already exists under the repository %d",
			artifactrsh.Digest, artifactrsh.RepositoryID); e != nil {
			err = e
		}
	}
	return id, err
...
func (d *dao) Filter(ctx context.Context) (arts []model.ArtifactTrash, err error) {
	...
	sql := fmt.Sprintf(`SELECT * FROM artifact_trash where artifact_trash.digest NOT IN (select digest from artifact_2) 
		AND manifest_media_type IN (%s,%s,%s,%s)`, v1.MediaTypeImageManifest, schema2.MediaTypeManifest, v1.MediaTypeImageIndex, manifestlist.MediaTypeManifestList)
