...
var (
	// Mgr is a global rbac policy manager instance
	Mgr = NewManager()
)

// Manager is the only interface of role permission module to provide the management functions for role permission.
type Manager interface {
	...
	// GetPermissionByRole get permissions by role type and id
...
func (m *manager) DeletePermissionByRole(ctx context.Context, role_type string, role_id int64) error {
	return m.dao.DeletePermissionByRole(ctx, role_type, role_id)
}
