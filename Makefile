...
ifeq ($(NOTARYFLAG), true)
	DOCKERSAVE_PARA+= vmware/notary-server-photon:$(NOTARYVERSION) vmware/notary-signer-photon:$(NOTARYVERSION) \
...
ifeq ($(CLAIRFLAG), true)
	DOCKERSAVE_PARA+= vmware/clair-photon:$(CLAIRVERSION) vmware/postgresql-photon:$(CLAIRDBVERSION)
...
modify_composefile: modify_composefile_notary modify_composefile_clair
	...
	@$(SEDCMD) -i 's/__reg_version__/$(REGISTRYVERSION)/g' $(DOCKERCOMPOSEFILEPATH)/$(DOCKERCOMPOSEFILENAME)
...
modify_composefile_notary:
	...
	@$(SEDCMD) -i 's/__notary_version__/$(NOTARYVERSION)/g' $(DOCKERCOMPOSEFILEPATH)/$(DOCKERCOMPOSENOTARYFILENAME)
...
modify_composefile_clair:
	...
	@$(SEDCMD) -i 's/__clair_version__/$(CLAIRVERSION)/g' $(DOCKERCOMPOSEFILEPATH)/$(DOCKERCOMPOSECLAIRFILENAME)
