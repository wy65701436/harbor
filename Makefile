...
GOBUILDPATH_REGISTRY=$(GOBUILDPATH)/src/registry
...
GOBUILDMAKEPATH_REGISTRY=$(GOBUILDMAKEPATH)/dev/registryctl
...
REGISTRYBINARYPATH=$(MAKEDEVPATH)/registryctl
REGISTRYBINARYNAME=harbor_registryctl
...
compile_golangimage: compile_clarity
	...
	@$(DOCKERCMD) run --rm -v $(BUILDPATH):$(GOBUILDPATH) -w $(GOBUILDPATH_REGISTRY) $(GOBUILDIMAGE) $(GOIMAGEBUILD) -o $(GOBUILDMAKEPATH_REGISTRY)/$(REGISTRYBINARYNAME)
	@echo "Done."
