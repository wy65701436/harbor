...
sudo wget https://bootstrap.pypa.io/get-pip.py && sudo python ./get-pip.py && sudo pip install --ignore-installed urllib3 chardet requests && sudo pip install pyasn1 google-apitools==0.5.15 gsutil==4.28 robotframework robotframework-httplibrary requests dbbot robotframework-pabot --upgrade
